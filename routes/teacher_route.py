...
from models.teacher_model import get_sem_date_active
...
def teacher_dashboard():
    ...
    try:
        user_id = session['teacher_id'].strip().lower()

        teacher = teacher_table(user_id)
        name = teacher['name'].strip().lower()
        department = teacher['department'].strip().lower()
        ...
        marked = teacher_marked_count(user_id, today)
        pending_classes, marked_classes=teacher_pending_count(today_class_count, marked)
...
def attendance_mark():
    ...
    try:
        ...
        if request.method == 'POST':        
            ...
            subject_name = request.form['subject_name'].strip().lower()
            department = request.form['department'].strip().lower()
            period = request.form['period_no']
            semester = request.form['semester']
            subject_code = request.form['subject_code'].strip().lower()
            teacher_id = request.form['teacher_id'].strip().lower()
...
def attendance_submit():
    ...
    try:
        if request.method == 'POST':
            ...
            if role == 'teacher':
                marked_by = session['teacher_id'].strip().lower()

            elif role == 'incharge':
                marked_by = session['incharge_id'].strip().lower()

            elif role == "hod":
                marked_by == session['hod_id'].strip().lower()
            else:
                return redirect(url_for('auth.login'))
            ...
            if action == 'submit':
                ...
                for key, value in request.form.items():
                    if key.startswith("status_"):
                        ...
                        insrt_attendance_tabel(register_no, name,department, subject_name, subject_code, teacher_id, attendance_date, period_no, status, marked_by)          
                ...
                for key, value in request.form.items():
                    if key.startswith("status_"):
                        ...
                        status = value
                        name = request.form.get(f"name_{register_no}")
                        update_attendance(register_no, name,department, subject_name, subject_code, teacher_id, attendance_date, period_no, status, marked_by)
...
def timetable():
    ...
    try:
        teacher_id = session['teacher_id'].strip().lower()
...
def attendance_report():
    ...
    try:
        ...
        year = None
        reports = {}
        active_semester = get_sem_date_active(department) 
        if not active_semester:
            active_semester = {}
        if request.method == "POST":
            semester =request.form['semester']

            students = get_report(semester)
            if students:
                register_no = [r['register_no'] for r in students]
                year = list(set(r['year'] for r in students))
                year = year[0] if year else None
                attendance_list = take_attendance_report(register_no, teacher_id)
...
def teacher_profile():
    ...
    try:
        teacher_id = session['teacher_id'].strip().lower()
        teacher = teacher_table(teacher_id)
        gender = teacher['gender'].strip().lower()
