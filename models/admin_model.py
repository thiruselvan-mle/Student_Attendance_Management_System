...
# --- CACHE POTTA FUNCTIONS (Static Counts & Data) ---
...
# --- CACHE PODATHA FUNCTIONS (Real-time Attendance) ---

def avg_attend(start_date, end_date):
    # அட்டெண்டன்ஸ் டேட்டா எப்போதும் ஃப்ரெஷ்-ஆக இருக்க வேண்டும்
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    try:
...
def std_for_each_dept():
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT department AS department, COUNT(*) AS total_students FROM students GROUP BY department"
...
# --- VALIDATIONS & OPERATIONS (No Cache) ---

def chech_dept(department_code):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT 1 FROM department WHERE department_code = %s"
...
def inst_dept(department_code, department_name):
    ...
    try:
        ...
        conn.commit()
        cache.delete_memoized(dept_lis) # டேட்டா மாறியதால் கேச் நீக்கவும்
        cache.delete_memoized(dept_count)
    finally:
        cursor.close()
        conn.close()
...
def delt_dept(department_code):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "DELETE FROM department WHERE department_code = %s"
        cursor.execute(query, (department_code,))
        conn.commit()
        cache.delete_memoized(dept_lis)
        cache.delete_memoized(dept_count)
    finally:
        cursor.close()
        conn.close()
...
def check_hod(department,hod_id):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT 1 FROM hod WHERE department = %s AND hod_id = %s"
...
def delt_hod(hod_id, department):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "DELETE FROM hod WHERE hod_id = %s AND department=%s"
...
def update_hod_for_admin(name, gender, email, phone, experience,start_year, end_year, password, hod_id, department):
    ...
    try:
        ...
        conn.commit()
        cache.delete_memoized(hod_lis)
        cache.delete_memoized(hod_data_for_view, hod_id, department)
    finally:
        cursor.close()
        conn.close()
...
def incharge_data_for_search(search_data):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT * FROM incharge WHERE (incharge_id LIKE %s OR name LIKE %s)"
        value = f"%{search_data}%"
...
def student_data_for_search(search_data):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT * FROM students WHERE (register_no LIKE %s OR name LIKE %s)"
        value = f"%{search_data}%"
        cursor.execute(query, (value, value))
        return cursor.fetchall() 
    finally:
...
def check_students(register_no, phone, email):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT 1 FROM students WHERE (register_no = %s OR email = %s OR phone =%s)"
...
def inst_to_students(register_no, email, name, phone, department, gender, password, year, semester, dob, admission_year, academic_year):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "INSERT INTO students(register_no, email, name, phone, department, gender, password, year, semester, dob, admission_year, academic_year) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        ...
        conn.commit()
        cache.delete_memoized(student_data)
        cache.delete_memoized(student_lis)
        cache.delete_memoized(std_for_each_dept)
    finally:
        cursor.close()
        conn.close()
...
def student_data_fro_update(register_no, email, name, phone, department, gender, password, dob,
    ...
    try:
        ...
        conn.commit()
        cache.delete_memoized(student_data)
        cache.delete_memoized(student_data_for_view, register_no)
    finally:
        cursor.close()
        conn.close()
...
# --- PROMOTION & DURATION (Critical - No Cache on Writes) ---

def promote_std():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute("SELECT register_no, name, department, academic_year, email, phone, year, semester FROM students")
        return cursor.fetchall()
    finally:
        cursor.close()
        conn.close()
...
def update_std_year_sem(register_no, year, semester):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute("UPDATE students SET year=%s, semester=%s WHERE register_no=%s", (year, semester, register_no))
        conn.commit()
        # ப்ரமோஷன் நடந்தால் அனைத்து மாணவர் கேச்-யும் நீக்க வேண்டும்
        cache.delete_memoized(student_data)
        cache.delete_memoized(std_for_each_dept)
    finally:
        cursor.close()
...
def update_sem_duration(year, semester, start_date, end_date):
    ...
    try:
        ...
        conn.commit()
        cache.delete_memoized(get_sem_duration, year, semester)
        cache.delete_memoized(sem_date)
    finally:
        cursor.close()
        conn.close()
...
def update_sem_mast_year(new_start_date, new_end_date, start_date, end_date):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE semester_master SET start_date = %s, end_date = %s WHERE start_date=%s AND end_date=%s",(new_start_date, new_end_date, start_date, end_date))
        conn.commit()
        cache.delete_memoized(year_from_sem_master)
    finally:
        cursor.close()
        conn.close()
...
# --- UTILITY CHECKS (Fresh Data Only) ---
