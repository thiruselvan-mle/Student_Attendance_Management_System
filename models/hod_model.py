    ...
    try:
        if department is None:
            ...
            query = """
                SELECT *
                FROM teachers
                WHERE teacher_year BETWEEN %s AND %s
            """
...
def check_teachers(teacher_id, email, phone, department):
    ...
    try:
        query = """
        SELECT 1 FROM teachers WHERE department = %s AND (teacher_id = %s OR email =%s OR phone =%s) LIMIT 1"""
    ...
    try:
        if department is None:
            ...
            query = """
                SELECT *
                FROM incharge
                WHERE incharge_year BETWEEN %s AND %s
            """
            cursor.execute(query, (start_year, end_year))
        else:
            # 🔹 Dept-wise HOD
...
def check_incharge(incharge_id, email, phone, department):
    ...
    try:
        query = """
        SELECT 1 FROM incharge WHERE  department = %s AND (incharge_id = %s OR email =%s OR phone =%s) LIMIT 1"""
...
def year_attendance(start_date, end_date, reg_no):
    ...
    try:
        ...
        query = f"""
            SELECT status, register_no, name
            FROM attendance
            WHERE attendance_date BETWEEN %s AND %s
            AND register_no IN ({placeholders})
        """
...
def teacher_subject_map(semester, year, department):
    ...
    try: 
        query = f"""SELECT subject_name, subject_code, teacher_id FROM subjects WHERE department = %s AND year =%s AND semester = %s"""
        cursor.execute(query, (department, year, semester))
        return cursor.fetchall()
...
def insert_timetable(day, department, period_no,
semester, subject_code, teacher_id, year, start_time, end_time):
    ...
    try: 
        query = f"""
                ...
                """
        cursor.execute(query, (day, department,
        period_no, semester, subject_code,
        subject_code, teacher_id, year,
        start_time, end_time))
        conn.commit()
...
def fetch_timetable(department, year, semester):
    ...
    try:
        cursor.execute("""
            SELECT day, period_no, subject_code, teacher_id
            FROM timetable
...
def fetch_assigned_subjects(department, year, semester):
    ...
    try:
        query = """
            SELECT subject_code, subject_name, teacher_id 
            FROM subjects 
            WHERE department = %s 
            AND year = %s 
            AND semester = %s 
    ...
    try:
        cursor.execute("""
            ...
            (subject_code, subject_name, teacher_id,
             department, year, semester)
            VALUES (%s,%s,%s,%s,%s,%s)
        """, (subject_code,subject_name,teacher_id,department,year,semester))
        conn.commit()
